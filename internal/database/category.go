...
func (c *Category) FindAllByDescription(query string) ([]Category, error) {
	rows, err := c.db.Query("SELECT id, name, description FROM categories where description like $1", "%"+query+"%")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
